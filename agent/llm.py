...
class LlmClient:
    ...
    def plan_and_code(
        ...
        combined_input = (
            ...
            "  `/demo-scrape-data?...`).\n"
            "- When loading tabular data (for example CSV files), be robust to whether the\n"
            "  first row is a header or data. If the first row looks numeric rather than\n"
            "  textual, treat it as data (for example, by using header=None or by explicitly\n"
            "  skipping header rows only when they are non-numeric).\n"
            ...
            "- Output only raw Python code. Do NOT wrap your code in Markdown fences such as "
            "  ```python ...```; the response must be directly executable by a Python interpreter.\n\n"
